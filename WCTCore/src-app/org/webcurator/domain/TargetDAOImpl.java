...
public class TargetDAOImpl extends BaseDAOImpl implements TargetDAO {
	...
	public Pagination search(final int pageNumber, final int pageSize, final Long searchOid, final String targetName, final Set<Integer> states, final String seed, final String username, final String agencyName, final String memberOf, final boolean nondisplayonly, final String sortorder) {
		return (Pagination) getHibernateTemplate().execute(
				new HibernateCallback() {
					public Object doInHibernate(Session session) {
						...
						if(targetName != null && !"".equals(targetName.trim())) {
							query.add(Restrictions.ilike("name", targetName + "%"));
							cntQuery.add(Restrictions.ilike("name", targetName + "%"));
						}
				...
				new HibernateCallback() {
					public Object doInHibernate(Session session) {
						...
						if(name != null && !"".equals(name.trim())) {
							...
							cntQuery.add(Restrictions.ilike("name", name + "%"));
						}
					...
					public Object doInHibernate(Session session) {
						...
						if(!nullOrEmpty(aPermissionCriteria.getSiteName())) {
							query.createCriteria("site")
								.add(Restrictions.ilike("title", aPermissionCriteria.getSiteName() + "%"));
						...
						if(!nullOrEmpty(aPermissionCriteria.getUrlPattern())) {
							query.createCriteria("urls")
								.add(Restrictions.ilike("pattern", aPermissionCriteria.getUrlPattern() + "%"));
				...
				new HibernateCallback() {
					public Object doInHibernate(Session session) {
				    	...
				    	List<Schedule> schedules = session.createCriteria(Schedule.class)
				    		...
				    		//.add(Restrictions.lt("lastProcessedDate", cal2.getTime()))
				    		//.setMaxResults(EnvironmentFactory.getEnv().getSchedulesPerBatch())
				    		.list();
