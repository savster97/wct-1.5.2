...
public class InTrayController extends AbstractFormController {
    ...
    protected ModelAndView showForm(HttpServletRequest aReq, HttpServletResponse aRes, BindException aErrors) throws Exception {
		...
		return defaultView(0, 0, Integer.parseInt(currentPageSize));
    }
    ...
    private ModelAndView viewNotification(InTrayCommand intrayCmd, int pageSize) {
        ...
        if (notifyOid != null) {
            ...
            log.warn("A form was posted to the InTrayController without a valid notifyOid attribute, redirecting to the showForm flow.");
            return defaultView(0, 0, pageSize);
        }
    }
    
    private ModelAndView deleteTask(InTrayCommand intrayCmd, int pageSize, BindException aErrors) {
        ...
        ModelAndView mav = defaultView(intrayCmd.getTaskPage(), intrayCmd.getNotificationPage(), pageSize);
    ...
    private ModelAndView defaultView(int taskPage, int notificationPage, int pageSize) {
        ModelAndView mav = new ModelAndView();
