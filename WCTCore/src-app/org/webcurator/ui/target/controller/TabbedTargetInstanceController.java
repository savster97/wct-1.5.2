...
public class TabbedTargetInstanceController extends TabbedController {
    ...
    protected ModelAndView processInitial(HttpServletRequest req, HttpServletResponse res, Object comm, BindException errors) {
		...
		if (req.getParameter("ti_oid") != null) {
			req.getSession().setAttribute(TargetInstanceCommand.SESSION_HH_TI_OID, req.getParameter("ti_oid"));
		} else {
			req.getSession().setAttribute(TargetInstanceCommand.SESSION_HH_TI_OID, "");
		}
		if (req.getParameter("harvestResultId") != null) {
			req.getSession().setAttribute(TargetInstanceCommand.SESSION_HH_HR_OID, req.getParameter("harvestResultId"));
		} else {
			req.getSession().setAttribute(TargetInstanceCommand.SESSION_HH_HR_OID, "");
		}
        ...
        req.getSession().setAttribute(Constants.GBL_SESS_EDIT_MODE, editMode); 
        
        
		//Code to display edit button (in layouts\tabbed-new.jsp) driven by session vars
        req.getSession().setAttribute(Constants.GBL_SESS_CAN_EDIT, false);
		if (!editMode)
		{
			...
			{
				req.getSession().setAttribute(Constants.GBL_SESS_CAN_EDIT, true);
			}	
		}
