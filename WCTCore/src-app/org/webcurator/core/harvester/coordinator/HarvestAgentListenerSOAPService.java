...
/**
 * The Server side implmentation of the HarvestAgentListener. This Service is
 * deployed on the core and is used by the agents to send messages to the core.
 * 
 * @author nwaight
 */
public class HarvestAgentListenerSOAPService extends ServletEndpointSupport
		implements HarvestAgentListener, CheckNotifier, IndexerService, DasCallback {
	/** the harvest coordinator to delegate to. */
	HarvestCoordinator hc = null;

	/** the logger. */
	private static Log log = LogFactory
			.getLog(HarvestAgentListenerSOAPService.class);

	protected void onInit() {
		hc = (HarvestCoordinator) getWebApplicationContext().getBean(
				Constants.BEAN_HARVEST_COORDINATOR);
	}
	...
	public void heartbeat(HarvestAgentStatusDTO aStatus) {
		if (log.isInfoEnabled()) {
			log.info("Received heartbeat from " + aStatus.getHost() + ":"
					+ aStatus.getPort());
		}
	...
	public void harvestComplete(HarvestResultDTO aResult) {
		if (log.isInfoEnabled()) {
			log.info("Received harvest complete for "
					+ aResult.getTargetInstanceOid() + " "
					+ aResult.getHarvestNumber());
		}
	 ...
	 *      String)
	 */
	public void notification(Long aTargetInstanceOid, int notificationCategory,
			String aMessageType) {
		if (log.isInfoEnabled()) {
			log.info("Received Notification TargetInstanceOid"
					+ aTargetInstanceOid + " with MessageType of "
					+ aMessageType);
		}
	 ...
	 *      String)
	 */
	public void notification(String aSubject, int notificationCategory,
			String aMessage) {
		if (log.isInfoEnabled()) {
	...
	public void addToHarvestResult(Long harvestResultOid, ArcHarvestFileDTO ahf) {
		try {
			log.info("Received addToHarvestResult(" + harvestResultOid + ", "
					+ ahf.getName() + ")");
	...
	public Long createHarvestResult(HarvestResultDTO harvestResultDTO) {
		try {
			log.info("Received createHarvestResult for Target Instance "
					+ harvestResultDTO.getTargetInstanceOid());
	...
	public void finaliseIndex(Long harvestResultOid) {
		try {
			log.info("Received finaliseIndex for Harvest Result "
					+ harvestResultOid);
	...
	public void notifyAQAComplete(String aqaId) {
		try {
			log.info("Received notifyAQAComplete for AQA Job "
					+ aqaId);
	...
	public void completeArchiving(Long targetInstanceOid, String archiveIID) {
		try {
			log.info("Received completeArchiving for Target Instance " + targetInstanceOid + " with archive IID of " + archiveIID);
