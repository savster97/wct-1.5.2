...
public class TargetManagerImpl implements TargetManager {
	...
	public void save(Target aTarget, List<GroupMemberDTO> parents) {
		...
		if( aTarget.isDirty() && 
			aTarget.getOriginalState() == Target.STATE_APPROVED &&
		...
		if( (aTarget.getOriginalState() == Target.STATE_PENDING 
			 || aTarget.getOriginalState() == Target.STATE_REINSTATED 
			 || aTarget.getOriginalState() == Target.STATE_NOMINATED
			 ) &&
		    aTarget.getState() == Target.STATE_APPROVED) {
		...
		if(originalState != aTarget.getState()) {
			...
			if(aTarget.getState() == Target.STATE_APPROVED ||
			   aTarget.getState() == Target.STATE_REJECTED) {
        ...
        if (numActiveTIsPrevious > 0 &&
        	targetInstanceDao.countActiveTIsForTarget(aTarget.getOid()) == 0 &&
        	aTarget.getOriginalState() == Target.STATE_APPROVED &&
        	aTarget.getState() == Target.STATE_APPROVED) {
	...
	public void createBatchTargetInstances(AbstractTarget aTarget, Schedule aSchedule, TargetGroup aGroup, boolean checkAgency) {
    	...
    	if( objectType == AbstractTarget.TYPE_TARGET || ( objectType == AbstractTarget.TYPE_GROUP && aGroup.getSipType() == TargetGroup.ONE_SIP ) ) {
    		...
    		if(checkAgency && !aTarget.getOwner().getAgency().equals(aSchedule.getOwningUser().getAgency())) {
		    	...
		    	if(startFrom == null) { startFrom = new Date(); }
	...
	/**
	 * @param instanceManager The instanceManager to set.
	 */
	...
	/**
	 * @param sendGroupUpdateNotifications The sendGroupUpdateNotifications to set.
	 */
	...
	/**
	 * @param subGroupParentTypesList The subGroupParentTypesList to set.
	 */
