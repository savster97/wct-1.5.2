	...
	public void check() {
		...
		if (percentage >= warnThreshold && !aboveWarnThreshold) {
			...
			if (log.isInfoEnabled()) {
				log.info("The used bandwidth has recovered below the warning threshold " + warnThreshold + "% and is " + percentage + "%");
			}
		}
		else if (percentage < errorThreshold && aboveErrorThreshold) {
			aboveErrorThreshold = false;
			if (log.isInfoEnabled()) {
				log.info("The used bandwidth has recovered below the error threshold " + errorThreshold + "% and is " + percentage + "%");
			}
		}
	}
