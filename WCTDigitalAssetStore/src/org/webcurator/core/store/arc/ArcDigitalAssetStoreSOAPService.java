...
public class ArcDigitalAssetStoreSOAPService extends ServletEndpointSupport implements DigitalAssetStoreSOAP   {
	...
	public DataHandler getResource(String targetInstanceName, int harvestResultNumber, HarvestResourceDTO resource) throws DigitalAssetStoreException {
		try {
			return new DataHandler(new TempFileDataSource(service.getResource(targetInstanceName, harvestResultNumber, resource)));	
		}
		catch(RuntimeException rex) {
			throw new DigitalAssetStoreException("SOAP Service Failed to get resource : " + rex.getMessage(), rex);
		}				
	}
	...
	public Header[] getHeaders(String targetInstanceName, int harvestResultNumber, HarvestResourceDTO resourcex) throws DigitalAssetStoreException {
		try {
		    return service.getHeaders(targetInstanceName, harvestResultNumber, resourcex);
        }
        catch(RuntimeException rex) {
            throw new DigitalAssetStoreException("SOAP Service Failed to get resource : " + rex.getMessage(), rex);
        }	
